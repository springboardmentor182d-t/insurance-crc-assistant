...
export default function FireRecResults() {
  ...
  useEffect(() => {
    ...
    api
      .post("/api/recommendations/fire", {
        property_type: state.property_type,
        occupancy_type: state.occupancy_type,
        construction_type: state.construction_type,
        property_age: state.property_age,

        fire: Boolean(state.fire),
        explosion: Boolean(state.explosion),
        lightning: Boolean(state.lightning),
        natural_disaster: Boolean(
          state.natural_disaster ?? state.natural
        ),
        burglary: Boolean(state.burglary),
        electronic_equipment: Boolean(
          state.electronic_equipment ?? state.electronics
        ),

        stock_value: Number(state.stock_value),
        machinery_value: Number(state.machinery_value),
        total_sum_insured: Number(state.total_sum_insured),
      })
      ...
      <div className="space-y-8">
        ...
        {!sortedPolicies.length && (
          <div className="text-center text-[var(--text-muted)] py-20">
            No matching fire policies found.
          </div>
        )}
      </div>
    </div>
