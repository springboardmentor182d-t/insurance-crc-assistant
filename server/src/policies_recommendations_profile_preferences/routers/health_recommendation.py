...
from src.policies_recommendations_profile_preferences.schemas.health_recommendation import HealthRecommendationInput
from src.policies_recommendations_profile_preferences.services.health_scoring import calculate_health_score

router = APIRouter()

@router.post("/")
def recommend_health(input: HealthRecommendationInput, db: Session = Depends(get_db)):
